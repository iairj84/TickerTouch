...
static void buildWeatherTab(lv_obj_t *parent) {
  ...
  lv_obj_align(condLbl, LV_ALIGN_TOP_RIGHT, 0, y + 2);
  y += 26;
  ...
  // ── Right side: feels like, H/L stacked next to big temp ──────────────
  ...
  lv_obj_align(feelsLbl, LV_ALIGN_TOP_RIGHT, 0, y);

  char hl[20]; snprintf(hl, sizeof(hl), "%.0f\xB0 / %.0f\xB0", w.highF, w.lowF);
  ...
  lv_obj_align(hlLbl, LV_ALIGN_TOP_RIGHT, 0, y + 18);
  y += 42;

  // ── Humidity + Wind on one clean line ─────────────────────────────────
  ...
  lv_obj_set_pos(hwLbl, 0, y); y += 20;

  // ── 3-day forecast cards ───────────────────────────────────────────────
  // Compute actual weekday names for forecast days
  ...
  int x = 0;
  for (int i = 0; i < 3; i++) {
    lv_obj_t *card = makeCard(parent, 150, 70);  // taller to fit condition
    ...
    lv_obj_set_pos(dayLbl, 0, 0);

    char fc[24];
    snprintf(fc, sizeof(fc), "%.0f\xB0 / %.0f\xB0",
      w.forecast[i].highF, w.forecast[i].lowF);
    lv_obj_t *fcLbl = lv_label_create(card);
    lv_label_set_text(fcLbl, fc);
    lv_obj_set_style_text_font(fcLbl, &lv_font_montserrat_16, 0);
    lv_obj_set_style_text_color(fcLbl, pal.text, 0);
    lv_obj_set_pos(fcLbl, 0, 16);

    lv_obj_t *condLbl = lv_label_create(card);
    lv_label_set_text(condLbl, wmoDescShort(w.forecast[i].code));
    lv_obj_set_style_text_font(condLbl, &lv_font_montserrat_12, 0);
    lv_obj_set_style_text_color(condLbl, pal.textMuted, 0);
    lv_obj_set_pos(condLbl, 0, 38);
  }
}
