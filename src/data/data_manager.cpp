...
static int httpGET_buf(const char *url, char *buf, int bufSize, int ms=10000) {
  ...
  int len = 0;
  if (code == 200) {
    // Read via 64-byte DRAM staging buffer.
    // readBytes() copies lwIP PSRAM → our stack chunk → our DRAM buf.
    // Using a chunk > 1 byte avoids chunked-transfer-encoding artifacts
    // that appear when reading the raw stream byte-by-byte.
    WiFiClient *stream = http.getStreamPtr();
    uint8_t chunk[64];  // stack = DRAM
    uint32_t deadline = millis() + ms;
    while (len < bufSize - 1 && millis() < deadline) {
      int avail = stream->available();
      if (avail > 0) {
        int rd = min(avail, (int)sizeof(chunk));
        rd = min(rd, bufSize - 1 - len);
        int got = stream->readBytes(chunk, rd);
        for (int i = 0; i < got; i++) buf[len++] = (char)chunk[i];
...
static int sportsFill(WiFiClient *stream, char *buf, int &bLen, int BSIZ, uint32_t deadline) {
  ...
  while (millis() < t && millis() < deadline) {
    int av = stream->available();
    if (av > 0) {
      int rd = min(av, (int)sizeof(tmp));
      rd = min(rd, BSIZ - 1 - bLen);
      if (rd <= 0) return 0;
      int got = stream->readBytes(tmp, rd);
      memcpy(buf + bLen, tmp, got);
      bLen += got;
      buf[bLen] = 0;
      vTaskDelay(1); // yield after each chunk — feeds watchdog during heavy reads
      return 1;
    }
...
void fetchSports() {
  ...
  for (auto &lg : LGS) {
    ...
    if (code != 200) {
      ...
      http.end();
      // On connection failure, give the TCP stack time to recover
      if (code < 0) vTaskDelay(pdMS_TO_TICKS(1000));
...
const char* buildTickerString() {
  ...
  if (weatherReady&&(Storage::cfg.widgetMask&WIDGET_WEATHER)) {
    char b[80]; snprintf(b,sizeof(b),"%s %.0fF H:%.0f L:%.0f %s    ",
      weather.city,weather.tempF,weather.highF,weather.lowF,weather.description);
    app(b);
  }
...
void fetchCalendar() {
  ...
  // Hard limit: 600KB — enough to cover most calendar histories.
  // Yield every 32KB to feed the watchdog without slowing reads too much.
  ...
  while (millis() < deadline && totalBytes < MAX_CAL_BYTES) {
    ...
    if (avail > 0) {
      ...
      if (totalBytes >= yieldAt) { yieldAt += 32768; vTaskDelay(1); }
    } else if (!stream->connected()) {
      if (linePos > 0) processLine();
      break;
    } else {
      vTaskDelay(1);
    }
  }
  // If we hit the byte limit, still flush last line
